...
wb = xl.load_workbook('DD.xlsx')  # opening workbook of excel sheet
...
x_Table_Var = [v1, v2, v3, v4, v5]

n = len(x_Table_Var)  # number of x points for n in the k1 line of best fit formula(Constant)
xSum = sum(x_Table_Var)  # sum total of x values for k1 line of best fit formula (Constant) (OK)
...
xl_data = []
for row in range(3, 1689):  # (x, y) x for starting position on excel row and y for final + 1
    for col in range(2, 7):  # (x, y) x for starting position on excel column (A,B,C) and y for final + 1
...
splits = np.array_split(xl_data, len(xl_data)/5)
...
splits3 = np.array_split(y_sum, len(xl_data)/5)
x_sum_y_sum = []
